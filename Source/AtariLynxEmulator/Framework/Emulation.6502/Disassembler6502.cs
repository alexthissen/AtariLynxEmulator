...
namespace KillerApps.Emulation.Processors
{
	public class Disassembler6502
	{
		...
		public static Instruction6502[] InstructionSet = new Instruction6502[256]
			{
				...
				null,														 
				new Instruction6502(0x20, "JSR", AddressingMode.Absolute),  
				new Instruction6502(0x21, "AND", AddressingMode.ZeroPageIndexedIndirectX), 
				null,														 
				null,														 
		...
		public ushort DisassembleSingleStatement(IMemoryAccess<ushort, byte> memory, ushort address, StringBuilder builder)
		{
			builder.AppendFormat("{0:X4}\t", address);
			byte opcode = memory.Peek(address++);
			builder.AppendFormat("{0:X2} ", opcode);
			
			Instruction6502 instruction = InstructionSet[opcode];

			if (instruction == null)
			{
				builder.AppendFormat("\t\t\t.db ${0:X2}\n", opcode);
				return 1;
			}
			...
			switch (instruction.AddressingMode)
			{
				...
				case AddressingMode.Relative: 
					ushort jump = (ushort)(address + 1 + operand8Bit);
					if (operand8Bit > 0x80) jump -= 256;
					builder.AppendFormat("{0:X2}\t\t{1} ${2:X4}\n", operand8Bit, instruction.Mnemonic, jump); 
					return 2;

				case AddressingMode.Immediate: builder.AppendFormat("{0:X2}\t\t{1} #${0:X2}\n", operand8Bit, instruction.Mnemonic); return 2;
				case AddressingMode.ZeroPage: builder.AppendFormat("{0:X2}\t\t{1} ${0:X2}\n", operand8Bit, instruction.Mnemonic); return 2;
				case AddressingMode.ZeroPageX: builder.AppendFormat("{0:X2}\t\t{1} ${0:X2},x\n", operand8Bit, instruction.Mnemonic); return 2;
				case AddressingMode.ZeroPageY: builder.AppendFormat("{0:X2}\t\t{1} ${0:X2},y\n", operand8Bit, instruction.Mnemonic); return 2;
				case AddressingMode.ZeroPageIndexedIndirectX: builder.AppendFormat("{0:X2}\t\t{1} (${0:X2},x)\n", operand8Bit, instruction.Mnemonic); return 2;
				case AddressingMode.ZeroPageIndirectIndexedY: builder.AppendFormat("{0:X2}\t\t{1} (${0:X2}),y\n", operand8Bit, instruction.Mnemonic); return 2;

				case AddressingMode.Absolute: builder.AppendFormat("{0:X2} {1:X2}\t{2} ${3:X4}\n", memory.Peek(address), memory.Peek((ushort)(address+1)), instruction.Mnemonic, operand16Bit); return 3;

...
namespace KillerApps.Emulation.Atari.Lynx.Tooling
{
	public partial class SpriteRenderControl : UserControl
	{
		...
		//int[] screenPixels = new int[160 * 102];
		...
		public void SetDefaultColorMap()
		{
			for (int i = 0; i < 16; i++)
			{
				...
				//engine.context.ColorMap[i] = (uint)(0x7F000000 + (0x00010101) * ((i + 1) * 15));
			}
		}
		...
		public void CopyColorMap()
		{
			for (int index = 0; index < 0x0F; index++)
			{
				...
				GreenColorMap[index] = (byte)(((value & 0x0F) << 4) * 15);

				value = LynxMemory[Mikey.Addresses.BLUERED0 + index];
				BlueColorMap[index] = (byte)((value & 0xF0) * 15);
				RedColorMap[index] = (byte)(((value & 0x0F) << 4) * 15);
			}
		}

		protected void PrepareRenderEngine()
		{
			LynxMemory = Engine.videoMemory;
			//CopyColorMap();
			SetDefaultColorMap();
		...
		{
			//screenPixels[counter] = (int)engine.context.ColorMap[source];
